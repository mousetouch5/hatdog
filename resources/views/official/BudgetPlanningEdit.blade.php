...
<form id="addTransactionForm" action="{{ route('budget.store') }}" method="POST">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        ...
        <div>
            ...
            <select id="year" name="year" class="w-full p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500" required>
    ...
    <div id="committee-container" class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="flex items-center justify-between bg-gray-100 p-3 rounded-md">
                ...
                <input type="text" name="{{ Str::snake($committee) }}" class="committee-input w-32 p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500" placeholder="₱0.00" />
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="mt-5">
        <button type="button" onclick="window.location.href='{{ route('Official.CalendarActivities.index') }}'" class="px-6 py-2 bg-gray-700 text-white rounded-md shadow hover:bg-gray-500 focus:ring-2 focus:ring-indigo-500">
            Calendar of Activities
        </button>
    </div>
    ...
    <div class="mt-8 text-right">
        <button type="button" class="px-6 py-2 bg-gray-700 text-white rounded-md shadow hover:bg-gray-500 focus:ring-2 focus:ring-indigo-500" onclick="document.getElementById('my_modal_2').showModal()">
            Save
        </button>
    </div>
    ...
    <dialog id="my_modal_2" class="modal">
        <div class="modal-box">
            ...
            <div class="modal-action">
                <!-- Cancel Button -->
                <button class="btn" type="button" onclick="document.getElementById('my_modal_2').close()">No</button>
                <!-- Confirm Button -->
                <!-- Yes Button -->
...
function updatePercentage(input, yearlyBudget) {
    ...
    if (!percentageDisplay || !percentageDisplay.classList.contains('percentage-display')) {
        ...
        percentageDisplay.classList.add('percentage-display', 'text-gray-500', 'ml-2', 'text-sm');
